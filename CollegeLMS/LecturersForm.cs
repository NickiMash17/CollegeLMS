...
namespace CollegeLMS
{
    public partial class LecturersForm : Form
    {
        ...
        private void LecturersForm_Resize(object sender, EventArgs e)
        {
            CenterHeader();
            CenterNavButtons();
            ApplyRoundedAll();
        ...
        private void LecturersForm_Paint(object sender, PaintEventArgs e)
        {
        ...
        private void LoadLecturers()
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    statusLabel.Text = "? " + currentTable.Rows.Count + " lecturers loaded";
                }
            }
            catch (Exception ex)
            {
                ...
                MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                try
                {
                    using (SqlConnection conn = new SqlConnection(connectionString))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@LecturerID", txtLecturerID.Text);
                        cmd.ExecuteNonQuery();
                        MessageBox.Show("??? Lecturer deleted!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        LoadLecturers();
                        ClearFields();
                    }
                }
